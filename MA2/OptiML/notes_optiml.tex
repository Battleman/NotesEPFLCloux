...
\defentry[Cauchy-Schwarz] $\|\mathbf{u}^\top \mathbf{v}\| \le |\mathbf{u}\| |\mathbf{v}| \to \frac{\mathbf{u}^\top \mathbf{v}}{\| \mathbf{u} \| \| \mathbf{v} \|} \in [-1,1] \sim \cos\alpha$
...
\defentry[Jensen's inequality] The above definition is valid for any number of points in $\dom(f)$. Any ``middle point'' will be in-between them, and always above $f$. Formally: Let $f$ be convex, and $x_1,\ldots,x_m \in \dom(f),\ \lambda_1,\ldots,\lambda_m \in \R_+$ such that $\sum_{i=1}^{m}\lambda_i = 1 $. Then \[
	...
	.\]
\defentry[Convex is continuous] Let f be convex, and suppose that $\dom(f)$ is open. Then $f$ is continuous.
...
\[
	\nabla^2 f(\mathbf{x}) \succeq 0\footnote{Componentwise inequality}
\]
...
\begin{enumerate}
		\item Do the analysis on the minimizer ($\mathbf{g}_t^\top (\mathbf{x}_t - \mathbf{x}^\star)$ )
...
\begin{itemize}
		\item (suppose 2D for simplicity) For any pair of points, the absolute value of the slope can't be greater than $B$.
		\item Take a cone, created by intersecting the slopes  $B$ and $-B$. Move this cone alongside the graph of $f$. If it is Lipschitz, the graph never crosses the cone. See Figure \ref{fig:lipschitz-cone} for reference; note how by moving the cone,  the graph never intersects the white cone.

\end{itemize}
\begin{figure}[htpb]
		\centering
		\includegraphics[width=0.8\columnwidth]{figures/lipschitz_cone}
		\caption{The cone moving alongside $f$}
		\label{fig:lipschitz-cone}
\end{figure}
...
\defentry[Operations on Smoothness]	Addition of smooth function is still smooth: $f_1, f_2,\ldots,f_m$ smooth with parameters $L_1,L_2,\ldots,L_m$ and let $\lambda_1, \lambda_2,\ldots,\lambda_m \in  \R_+$. Then $f := \sum_{i=1}^{m} \lambda_i f_i $ is smooth, with parameter $\sum_{i=1}^{m} \lambda_i L_i $.
...
\[
	\Pi_X(\mathbf{y}) := \argmin_{\mathbf{x} \in  X} \|\mathbf{x}-\mathbf{y}\|^2
\]
Idea: compute first what would be your next step ($\mathbf{y}_{t+1}$) and then project it on your actual next step. \[
	...
	\end{array}
\]
\defentry[Properties] Let $X \subseteq  \R^d$ be closed and convex, $\mathbf{x} \in  X, \mathbf{y} \in  \R^d$. Then
\begin{itemize}
		\item  $\big(\mathbf{x} - \Pi_X(\mathbf{y})\big)^\top \big(\mathbf{y}-\Pi_X(\mathbf{y})\big) \leq 0$
...
\defentry[Equivalent proofs of projected] The last property above needs to be proved (not here). However, here are some insights: We now only need the function to be continuous over $X$, and not anymore $\R^d$. Then, in the proofs we replace $\mathbf{x}_{_t+1}$ by  $\mathbf{y}_{t+1}$, and massage it with the facts above.
...
\defentry[Proximal gradient] Classical gradient step: \[
		\begin{array}{lll}
				\mathbf{x}_{t+1} &= &\argmin\limits_\mathbf{y} g(\mathbf{x}_t) + \nabla g(\mathbf{x}_t)^\top (\mathbf{y}-\mathbf{x}_t)\\
						  &  &+ \frac{1}{2\gamma}\|\mathbf{y}-\mathbf{x}_t\|^2\\
						  &= &\argmin\limits_{\mathbf{y} \in \R^d} \frac{1}{2\gamma} \|\mathbf{y} - (\mathbf{x}_t - \gamma \nabla g(\mathbf{x}_t))\|^2
		\end{array}
\] For $f = g+h$ keep the same for $g$ and add $h$ unmodified:\[
	\begin{array}{l}
			\mathbf{x}_{t+1} =\\ 
			\argmin\limits_\mathbf{y}  \overbrace{g(\mathbf{x}_t) + \nabla g(\mathbf{x}_t)^\top (\mathbf{y}-\mathbf{x}_t) + \frac{1}{2\gamma}\|\mathbf{y}-\mathbf{x}_t\|^2}^{\text{Quadratic upperbound from smooth }g} \\
									   + \underbrace{h(\mathbf{y})}_{\text{Non smooth}} \\
...
For GD: we assumed $\|\nabla f(\mathbf{x})\|^2 \leq B^2_{GD}$, leading to \[
...
\defentry[Constrained optimization] By slapping a projection back to $X$ onto every step, we obtain projected SGD, with the same convergence rate as above.
...
\begin{lemma*}
	Suppose that $\mathbf{x} > \mathbf{0}$ is c-balanced. Then for any $I \subseteq \{1,\ldots,d\}$, we have \[
		\left( \frac{1}{c} \right)^{|I|} \left( \prod\limits_k x_k \right)^{1- \frac{|I|}{d} }
					\leq \prod\limits_{k \not\in I} x_k
					\leq c^{|I|} \left( \prod\limits_k x_k \right)^{1-\frac{|I|}{d}}
	\]
\end{lemma*}
...
\begin{theorem*}
		Every first-order method needs in the worst case $\Omega(1/\sqrt{\epsilon})$ steps (gradients evaluations) in order to achieve an additive error of $\epsilon$ on smooth functions.
\end{theorem*}
...
\defentry[Constructing a quasi-newton method] %TODO
\defentry[Greenstadt's family of QN methods] %TODO 
\section{Frank-Wolfe}
...
\begin{itemize}
  ...
  \item \textbf{Generalized linear models} $\min\limits_{\mathbf{x}\in \R^d} f(A\mathbf{x}) + g(\mathbf{x})$. We reformulate as $\min\limits_{\mathbf{x} \in \R^d, \mathbf{w} \in \R^n} f(\mathbf{w}) + g(\mathbf{x})$, such that $\mathbf{w} = A\mathbf{x}$. We define the Lagrange dual function: \[
				  \begin{array}{ll}
...
\defentry[Decentralized learning] An interesting alternative, where devices create some kind of mesh (not fully connected). Instead of sending everything (data or gradients) to a centralized point, just communicate between devices. Motivation: sensible data, not to be gathered at one place. Simplest consensus: share your vector with neighbours, and everyone compute means of their neighbours.
\defentry[Compression] Still, sending data is often heavy. Idea: send compressed data (some bits per entry, or some top\% for each vector,\ldots Just sending the compressed data and taking average will systematically converge \textit{wrongly}. Other new methods (developed by MLO) work better
\defentry[Decentralized algo] 
\begin{enumerate}
		\item SGD step:  $\mathbf{x}^i_{t + \frac{1}{2}} := \mathbf{x}^j_t - \gamma_t \nabla f^j_{i_t}(\mathbf{x}^j_t)$
		\item Average step: $\mathbf{x}^i_{t+1}:= \frac{1}{\text{deg}_i} \sum\limits_{j:\text{neighbours}} \mathbf{x}^j_{t+ \frac{1}{2}}$
\end{enumerate}
\defentry[CHOCO-SGD] Same as before, but with compression. Use consensus with compression on the SGD step to compute the next iterate. Convergence is in \[
		\frac{1}{T+1} \sum_{t=0}^{T}\|\nabla f(\overline{\mathbf{x}_t}\|^2 = \O\left( \frac{1}{\sqrt{nT} } + \frac{n}{\delta^2 \rho^{4}T} \right)  
\] 
With $n$ the number of workers, $\delta$ the compression rate $\in \left[ 0,1 \right] $ (1 for no compression), and $\rho$ the spectral gap of the graph topology. So we have a linear speedup in the number of workers. 
...
\defentry[How to prove convex] There are many ways to prove convexity.
\begin{itemize}
		\item By definition: $f(\lambda \mathbf{x} + (1-\lambda)\mathbf{y}) \le \lambda f(\mathbf{x}) + (1-\lambda)f(\mathbf{y})$
		\item By composition: Sum/composition of convex functions is convex, or multiplication by a real. Careful, may change the domain.
		\item By first order characterization: graph must be above all its tangent hyperplanes $f(\mathbf{y}) \ge f(\mathbf{x}) + \nabla f(\mathbf{x})^\top (\mathbf{y}-\mathbf{x})$. Bidirectional relation.
		\item By second order characterization: convex $\iff$ hessian (strictly) positive componentwise for all points in the domain (if open domain, hessian exists + symmetric). If the function is 1D, only check that the second derivative is positive for all points in the domain.
\end{itemize}

\defentry[How to prove continuity]
\begin{itemize}
		\item Convex + open domain $\to $ continuous
		\item Differentiable on $\left[ a,b \right] \to $ continuous on $\left[ a,b \right] $. For particular points $a$, use definition: $\lim\limits_{x \to a} \frac{f(x) - f(a)}{x-a}$ (from both sides, ideally).
\end{itemize}
\section{Tables and summaries}
\begin{table}[htpb]
	\centering
	\begin{tabular}{c|c|c|c|c}
			& \thead{Lipschitz\\convex} & \thead{Smooth\\convex} & \thead{Strongly\\convex} & \thead{Smooth \&\\ Strongly convex}\\
			\hline
			GD & $\O\left( 1/\epsilon^2 \right) $ & $\O(1/\epsilon)$ & & $\O(\log(1/\epsilon))$ \\
			Accelerated& & $\O(1/\sqrt{\epsilon})$ &  \\
			Projected & $\O(1/\epsilon^2)$ & $\O\left( 1/\epsilon \right) $ & & $\O\left( \log(1/\epsilon) \right) $ \\
			Proximal & & $\O(1/\epsilon)$ & \\
			Subgradient& $\O\left( 1/\epsilon^2 \right) $ & & $\O\left( 1/\epsilon \right) $ \\
			SGD & $\O(1/\epsilon^2)$ & & $\O\left( 1/\epsilon \right) $
	\end{tabular}
