...
\begin{itemize}
		\item  $\big(\mathbf{x} - \Pi_X(\mathbf{y})\big)^\top \big(\mathbf{y}-\Pi_X(\mathbf{y})\big) \leq 0$. This means that the angle $\alpha$ between $\mathbf{x}$ and $\mathbf{y}$ at the point of the projection is always $\ge 90^\circ$
		\item $\|\mathbf{x} - \Pi_X(\mathbf{y})\|^2 + \|\mathbf{y}-\Pi_X(\mathbf{y})\|^2 \leq \|\mathbf{x}-\mathbf{y}\|^2$. The squared distance $\mathbf{x} \to \mathbf{y}$ is greater than the sum of the squared catheti. If we were to remove the squares, then inequality shifts (Pythagoras). If angle is a right angle, it becomes an equality. Use first property and  $2\mathbf{v}^\top \mathbf{w}$ to prove.
\end{itemize}
...
\] For $f = g+h$ keep the same for $g$ and add $h$ unmodified:\[
	\begin{array}{lll}
			\mathbf{x}_{t+1} & = & \argmin\limits_\mathbf{y} \overbrace{g(\mathbf{x}_t)^\top (\mathbf{y}-\mathbf{x}_t) + \frac{1}{2\gamma}\|\mathbf{y}-\mathbf{x}_t\|^2}^{\text{Quadratic upperbound from smooth }g} \\
